...
const y = await p.evaluate(() => { const s=[...document.querySelectorAll("section")].find(x=>/@arazwoodenconcept|Follow along|Bizi takip/.test(x.textContent||"")); return s?s.getBoundingClientRect().top+window.scrollY:6000; });
await p.evaluate((yy)=>window.scrollTo(0,yy-90), y);
await new Promise((r)=>setTimeout(r,1400));
await p.screenshot({ path: OUT + "social.png" });
await b.close();
console.log("done", y);
